...
@login_required
def api_users():
    """Return list of IPs with active connections on the public WebSDR port.

    Uses 'ss -tn sport = :PORT' (no -p flag; no privilege needed).
    The port scanned is the PUBLIC port (default 8900) — the port users
    actually connect to (proxy.py or spectrumserver direct).
    Do NOT use spectrumserver's internal port (e.g. 8902): those connections
    all come from 127.0.0.1 (proxy.py) and real IPs are invisible there.
    """
    ...
    try:
        ...
        return jsonify({"ok": True, "users": users, "port": port})
    except Exception as e:
...
@login_required
def api_kick():
    """Close all TCP connections from a given IP via 'ss -K'.

    The kill runs in a background thread AFTER the HTTP response is sent.
    This prevents the admin browser losing its own connection if it shares
    the same external IP as the user being kicked (NAT / same network).

    Requires: sudo setcap cap_net_admin+ep $(which ss)
    """
    data = request.get_json(silent=True) or {}
    ...
    if ip.startswith("::ffff:"):
        ip = ip[7:]  # normalize IPv4-mapped IPv6 so ss can match it
    # Normalize IPv4-mapped IPv6 (::ffff:1.2.3.4 → 1.2.3.4) so ss can match it
    if ip.startswith("::ffff:"):
        ip = ip[7:]
    ...
    try:
        # Count connections before kick to validate ss has permission
        ...
        # ── Fire the kick in a background thread 300 ms from now ──────────────
        # Sending the response first ensures the browser receives "ok" before
        # ss -K potentially terminates the admin's own TCP connection to the proxy.
        def _kick_later():
            import time
            time.sleep(0.3)
            try:
        ...
        return jsonify({
            ...
            "msg": "Kicked %s — %d connection%s terminated" % (
                ip, count_before, "s" if count_before != 1 else ""),
        })
    except Exception as e:
        return jsonify({"ok": False, "msg": str(e)})
