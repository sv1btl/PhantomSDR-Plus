...
#pragma omp parallel for simd
    for (size_t i = 0; i < outbuf_len; i++) {
        ...
        powerbuf[i] = power;
        quantizedbuf[i] = static_cast<int8_t>(std::clamp(
...
int clFFT::plan_r2c(int) {
    // NOTE: outbuf and quantizedbuf are mapped directly from cl_outbuf /
    // cl_quantizedbuf below.  Do NOT call this->malloc() here — any host
    // pointer returned by that path is added to the buffers[] map and then
    // immediately orphaned when outbuf is overwritten, leaking the CL buffer.
