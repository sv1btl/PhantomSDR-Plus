...
$SUDO pacman -Sy --needed --noconfirm base-devel cmake pkg-config meson fftw websocketpp flac zlib zstd boost opus liquid-dsp git curl cargo nlohmann-json

# Install curlpp from AUR if not available
if ! pacman -Qi curlpp >/dev/null 2>&1; then
    echo ""
    echo "⚠️  Note: curlpp is not in the official repos."
    echo "You may need to install it from AUR:"
    echo "  yay -S curlpp"
    echo "or"
    echo "  paru -S curlpp"
    echo ""
...
if [ $? -eq 0 ]; then
    ...
    case $option in
        ...
        2) echo "Setting up RTLSDR..."
            ...
            if [[ $rtlsdr_v4 == "y" || $rtlsdr_v4 == "Y" ]]; then
                ...
                $SUDO pacman -S --needed --noconfirm libusb git cmake pkg-config
                ...
                $SUDO pacman -S --needed --noconfirm rtl-sdr
                echo "Standard RTLSDR drivers installed successfully."
            fi
            ;;
        3) echo "Setting up SDRPlay..."
            echo ""
            echo "⚠️  Note: libmirisdr is available in AUR."
            echo "You need to install it manually using an AUR helper:"
            echo "  yay -S libmirisdr-git"
            echo "or"
            echo "  paru -S libmirisdr-git"
            echo ""
            read -p "Press Enter to continue after installing libmirisdr..."
            ;;
    ...
    if [[ $install_opencl == "y" || $install_opencl == "Y" ]]; then
        ...
        $SUDO pacman -S --needed --noconfirm ocl-icd opencl-headers clinfo
        ...
        if [ $? -ne 0 ]; then
            ...
            if [[ $cpu_vendor == "GenuineIntel" ]]; then
                ...
                echo "For AMD GPUs, install: sudo pacman -S rocm-opencl-runtime"
                echo "For NVIDIA GPUs, install: sudo pacman -S opencl-nvidia"
                echo ""
