...
$SUDO dnf install -y @development-tools cmake pkg-config meson fftw-devel websocketpp-devel flac-devel zlib-devel libzstd-devel boost-devel opus-devel liquid-dsp-devel git curlpp-devel curl cargo json-devel
...
if [ $? -eq 0 ]; then
    ...
    case $option in
        ...
        2) echo "Setting up RTLSDR..."
            ...
            if [[ $rtlsdr_v4 == "y" || $rtlsdr_v4 == "Y" ]]; then
                ...
                $SUDO dnf remove -y 'rtl-sdr*' 'librtlsdr*'
                $SUDO rm -rvf /usr/lib/librtlsdr* /usr/include/rtl-sdr* /usr/local/lib/librtlsdr* /usr/local/include/rtl-sdr* /usr/local/include/rtl_* /usr/local/bin/rtl_*
                $SUDO dnf install -y libusb-devel git cmake pkg-config
        ...
        3) echo "Setting up SDRPlay..."
            $SUDO dnf install -y libmirisdr-devel
            echo "SDRPlay libraries installed successfully."
            ;;
    ...
    if [[ $install_opencl == "y" || $install_opencl == "Y" ]]; then
        ...
        $SUDO dnf install -y ocl-icd ocl-icd-devel opencl-headers clinfo
        ...
        if [ $? -ne 0 ]; then
            ...
            echo ""
            echo "⚠️  Note: Intel Compute Runtime installation on Fedora requires manual steps."
            echo "Please follow these instructions:"
            echo ""
            echo "1. For Intel CPUs with integrated graphics (iGPU):"
            echo "   sudo dnf install -y intel-opencl"
            echo ""
            echo "2. For Intel CPUs without iGPU (requires NEO driver):"
            echo "   Visit: https://github.com/intel/compute-runtime/releases"
            echo "   Download and install the appropriate RPM packages for your Fedora version"
            echo ""
            ...
            if [ $? -eq 0 ]; then
                ...
                echo ""
                echo "⚠️  IMPORTANT: You may need to REBOOT your system for OpenCL to work properly."
                NEEDS_REBOOT=true
            else
    ...
    case $option in
        ...
        3) echo "✅ SDR Hardware: SDRPlay"
           echo "   • libmirisdr-devel installed"
           echo "" ;;
