#!/bin/bash
# ─────────────────────────────────────────────────────────────────────────────
#  PhantomSDR-Plus  –  start-fobos-hf.sh
#  Universal launcher + watchdog for a RigExpert Fobos SDR, HF1/HF2 direct sampling 0-25 MHz (via SoapySDR rx_sdr).
#
...
#
#  Usage:
#    ./start-fobos-hf.sh           start (or restart) the server in the background
...
#
#  Drivers: ./setup-fobos.sh (or install.sh option 5) builds SoapyFobosSDR,
#  rx_sdr and cf32_to_real. The Fobos streams ONE path at a time, RF or HF —
#  start-fobos.sh and start-fobos-hf.sh share the watchdog lock like every
#  start-*.sh, so starting one stops the other.
#
#  NOTE: the rx_sdr arguments and config-fobos-hf.toml were validated on a
#  Fobos by another station; the control/watchdog logic is the one validated
#  on RX-888. The 50 Msps FFT wants a GPU — see accelerator in the .toml.
# ─────────────────────────────────────────────────────────────────────────────
...
RX_LABEL="Fobos SDR HF (direct sampling)"
RX_COMM="rx_sdr"                                   # process name to monitor/kill
CONFIG="$PHANTOMDIR/config-fobos-hf.toml"
FIFO="$PHANTOMDIR/fobos_hf.fifo"
# -f 0 is intentional: direct sampling has no local oscillator, and rx_sdr's
# "Failed to set center freq" warning is expected. 50 Msps covers 0-25 MHz.
RX_ARGS="${RX_ARGS:--d driver=fobos -t direct_samp=1 -f 0 -s 50000000 -I CF32 -F CF32 -}"
# rx_sdr | cf32_to_real, which keeps I and scales it to s16 (format="s16").
# $0 is the converter, "$@" are RX_ARGS. Only rx_sdr needs watching: if the
# converter dies rx_sdr takes SIGPIPE, and if rx_sdr dies the converter ends.
RX_CMD=(bash -c 'rx_sdr "$@" | "$0"' "$PHANTOMDIR/cf32_to_real")
prestart() {
    [ -x "$PHANTOMDIR/cf32_to_real" ] \
        || log "ERROR: $PHANTOMDIR/cf32_to_real not found — run ./setup-fobos.sh"
}
# ═════════════════════════════════════════════════════════════════════════════
