...
AGC::AGC(float desiredLevel, float attackTimeMs, float releaseTimeMs, float lookAheadTimeMs, float sr)
    ...
    attack_coeff  = 1.0f - std::exp(-1.0f / (attackTimeMs  * 0.002f * sample_rate));
    ...
    max_gain = 500.0f;  // → max_lin_gain ≈ 5x (~+8 dB)
...
void AGC::configureForAM() {
    ...
    am_attack_coeff  = attack_coeff * 0.5f;                     // tight peak control
    am_release_coeff = release_coeff * 0.15f;                   // very slow gain-up; no “breathing”
}
