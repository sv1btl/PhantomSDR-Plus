...
# =============================================================================
# DISTRIBUTION CHECK — this variant is only for Ubuntu 22.04
# =============================================================================
# The mirror of the check in install_rade.sh. This variant exists solely because
# Jammy's apt python3-websockets is 10.1 and rade_helper.py needs >= 11.0; on
# any newer release the apt package is fine and install_rade.sh is the one to
# run, so pip should not be pushed in front of the distribution's own package.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MAIN_SCRIPT="$SCRIPT_DIR/install_rade.sh"
...
if [[ -n "$OS_ID" && ! ( "$OS_ID" == "ubuntu" && "$OS_VER" == 22.04* ) ]]; then
    section "This is the Ubuntu 22.04 variant"
    echo ""
    echo "  Detected: ${OS_ID} ${OS_VER:-(unknown version)}"
    echo ""
    echo "  This variant only exists to work around Ubuntu 22.04's outdated"
    echo "  python3-websockets (10.1). On your system the normal installer,"
    echo "  install_rade.sh, is the right one."
    echo ""
    if [[ -f "$MAIN_SCRIPT" ]]; then
        read -rp "Switch to install_rade.sh now? [Y/n]: " use_main
        if [[ ! ${use_main:-y} =~ ^[Nn] ]]; then
            info "Handing over to install_rade.sh …"
            echo ""
            exec bash "$MAIN_SCRIPT" "$@"
        fi
        warn "Continuing with the Ubuntu 22.04 variant on ${OS_ID} ${OS_VER:-(unknown version)}"
    else
        warn "install_rade.sh not found next to this script — continuing"
    fi
fi
...
sudo env DEBIAN_FRONTEND=noninteractive apt-get install -y \
    ...
    python3-scipy \
    python3-matplotlib \
    python3-psutil   # psutil is used by rade_loadtest.py (concurrency benchmark)
