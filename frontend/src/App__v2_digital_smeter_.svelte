...
// amkamk
  ...
  function _smeterTick() {
    // Raw calibrated power in dBm (same calibration knob as the analog variants)
    let rawDb = audio.getPowerDb();
    const analogSmeterOffset = Number.isFinite(Number(audio?.analog_smeter_offset))
      ? Number(audio.analog_smeter_offset)
      : 0;
    rawDb += analogSmeterOffset;
    try {
      window._lastPowerDb = rawDb;
      eyeDbm = rawDb;
      // Purely spectral SNR from off-passband spectrum bins (waterfall.js).
      // Noise floor is derived so (dBm − NF) === SNR by construction.
