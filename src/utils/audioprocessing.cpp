...
AGC::AGC(float desiredLevel, float attackTimeMs, float releaseTimeMs, float lookAheadTimeMs, float sr)
    ...
    release_coeff = 1 - exp(-1.0f / (releaseTimeMs * 0.009f * sample_rate)); //was 0.001f
    ...
    // Hang system
    hang_time = static_cast<size_t>(0.2f * sample_rate); // Was 0.05f * sample_rate
    hang_threshold = 0.1f; // was 0.05f

    // Dual time constant
    fast_attack_coeff = 1 - exp(-1.0f / (0.5f * 0.001f * sample_rate));
    
    // AM time constants
    am_attack_coeff = attack_coeff * 0.5f; // was 0.1
    am_release_coeff = release_coeff * 0.5f; //was 0.1

    // Initialize Noise Blanker parameters
    nb_enabled = false;
    nb_fft_size = 2048; //was 512
    nb_overlap = 1536; //was 256
    nb_average_windows = 32; //was 32
    nb_threshold = 180.0f; //was 95
