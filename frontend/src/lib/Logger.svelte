<script>
    ...
    export function addLine (text) {
      ...
      // BUG FIX (MEMORY LEAK / CORRUPTED LOG):
      // log.unshift() with no argument calls Denque.prototype.unshift(undefined),
      // which adds `undefined` to the FRONT of the queue.  The log never shrank,
      // grew without bound, and rendered blank lines for every overflow event.
      // The correct Denque API to drop the oldest (front) entry is shift().
