...
├── demo_installer.sh          # холостой прогон install.sh — показывает ход, ничего не ставит
...
| `demo_installer.sh` | Холостой прогон `install.sh`: показывает весь ход и ничего не устанавливает |
...
Метки частот на водопаде. `mymarkers.json` — это список, который приёмник действительно
показывает; остальное — сырьё, которое `update-markers.sh` в него превращает, обновляемое
из сетевых расписаний. `README.md` в этом каталоге объясняет обновление на всех семи языках.
