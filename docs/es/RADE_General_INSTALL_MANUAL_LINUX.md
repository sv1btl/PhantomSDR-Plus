...
**Normalmente no necesita un script de control aparte para RADE.** Los scripts de arranque (`start-rx888mk2.sh`, `start-airspyhf.sh`, `start-rtl.sh`, `start-rsp1a.sh`) arrancan el sidecar por sí mismos en cuanto el servidor está en marcha, lo reinician si termina e informan de su estado; `stop-websdr.sh` lo detiene junto con el servidor. Pase directamente al Paso 7.
