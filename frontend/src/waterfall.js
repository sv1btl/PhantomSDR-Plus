  ...
  calculate(magnitudes) {
    ...
    if (this.config.adaptiveEnabled) {
      ...
      if (now - this.lastAdaptation >= this.config.adaptationInterval) {
        ...
        // BUG FIX (PERFORMANCE): console.log fired every 1 second in the
        // rendering thread, causing constant string allocation and GC pressure.
        // Removed from the hot path; uncomment temporarily for diagnostics:
        // console.log(`🔄 ${result.condition} | SNR: ${result.avgSNR.toFixed(1)} dB | ...`);
      }
    }
...
//This copied "as is" by sv2amk from below to fix the same issue but for the modes.//
  ...
  _getFlagImg(cc) {
    ...
    const url = `https://cdn.jsdelivr.net/npm/@twemoji/api@15.1.0/assets/72x72/${cp(key[0])}-${cp(key[1])}.png`
    const img = new Image()
