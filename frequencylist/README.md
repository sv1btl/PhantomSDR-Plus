...
- download from https://new.hfcc.org/data/ the newest list, at the buttom of page, current: **a26allx2.zip** - a26 Operational Schedule - **Last updated on 14-Apr-2026** 
...
- run update script with "update-markers.sh" manually or let it run automaticly by a cron job (for example update every 15 minutes a hour 'crontab -e' and enter '0,15,30,45 * * * * /usr/bin/bash /home/sv1btl/PhantomSDR-Plus/frequencylist/update-markers.sh >> /var/log/frequencylist.log 2>&1'). Please note that "sv1btl" must be YOUR user name!
