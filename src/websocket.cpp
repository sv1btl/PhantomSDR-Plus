...
void broadcast_server::on_open(connection_hdl hdl) {
    server::connection_ptr con = m_server.get_con_from_hdl(hdl);
    // get_resource() includes any query string (e.g. "/audio?tap=abc"). Split it
    // so routing still matches on the bare path and the loopback exemption can
    // read the ?tap=<token> parameter.
    ...
    std::string query;
    {
        ...
        if (qpos != std::string::npos) {
            ...
            query = resource.substr(qpos + 1);
        }
    }

    // BUG FIX (all 5 loopback bugs) — check FIRST, before any resource allocation:
    //
    // 1. ORDER: the old check was AFTER the ping-timer loop was started (lines
    //    below).  For a loopback connection, the timer fired every 25 s forever
    //    because the connection was never closed — a permanent timer leak.
    //
    // 2. CONNECTION NOT CLOSED: the old code just did `return` inside the /audio
    //    block without calling m_server.close(), leaving the half-open connection
    //    with no message/close handler and the ping timer running.
    //
    // 3. REDUNDANT get_con_from_hdl: `con` is already available; the old code
    //    called get_con_from_hdl(hdl) a second time inside the filter block,
    //    which can throw and bypass the filter entirely if the connection races.
    //
    // 4. ONLY /audio FILTERED: loopback connections to /waterfall, /events, and
    //    /chat were not filtered at all — useless bandwidth and timer waste.
    //
    // 5. WRONG IP SOURCE: the old filter used get_remote_endpoint() directly,
    //    but ip_from_hdl() honours X-Forwarded-For / X-Real-IP first.  When
    //    Nginx proxies the connection, get_remote_endpoint() is always 127.0.0.1
    //    even for real remote users, so the old filter would have falsely blocked
    //    every connection through an Nginx reverse proxy.  Use ip_from_hdl() so
    //    the loopback check is consistent with what ends up in ip_address.
    {
        const std::string client_ip = ip_from_hdl(hdl);
        const bool loopback =
            client_ip == "127.0.0.1"                  ||
            client_ip == "::1"                         ||
            client_ip.find("::ffff:127.") != std::string::npos ||
            // Fallback: also check the raw TCP endpoint in case ip_from_hdl
            // returns a proxy header value while the actual socket is loopback.
            con->get_remote_endpoint().find("127.0.0.1") != std::string::npos ||
            con->get_remote_endpoint().find("[::1]")      != std::string::npos;

        if (loopback) {
            // Exempt ONLY an authenticated internal PCM tap: a loopback client
            // on /audio that presents the correct ?tap=<token>. Everything else
            // loopback is still closed. The token is a per-boot secret written
            // to ./.tap_token (mode 600), so only a same-host process running as
            // this user can obtain it.
            bool tap_ok = false;
            if (path == "/audio" && !tap_token.empty()) {
                // Extract the value of the "tap" query parameter.
                const std::string key = "tap=";
                size_t p = query.find(key);
                // Must be at the start or preceded by '&' to avoid matching e.g. "xtap=".
                while (p != std::string::npos && p != 0 && query[p - 1] != '&') {
                    p = query.find(key, p + 1);
                }
                if (p != std::string::npos) {
                    size_t vstart = p + key.size();
                    ...
                    if (val.size() == tap_token.size() && val == tap_token) {
                        tap_ok = true;
                    }
                }
            }
            if (!tap_ok) {
                // Close the connection cleanly — do NOT just return.
                // A bare return would leave the connection half-open with no handlers.
                websocketpp::lib::error_code ec;
                m_server.close(hdl, websocketpp::close::status::going_away, "", ec);
                return;
            }
