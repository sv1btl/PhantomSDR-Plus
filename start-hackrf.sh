#!/bin/bash
# ─────────────────────────────────────────────────────────────────────────────
#  PhantomSDR-Plus  –  start-hackrf.sh
#  Universal launcher + watchdog for a HackRF One (via hackrf_transfer).
#
...
#
#  Usage:
#    ./start-hackrf.sh             start (or restart) the server in the background
...
#    RX_ARGS="…"                  override the hackrf_transfer argument string
#
#  Driver: ./setup-hackrf.sh (or install.sh option 7) installs the
#  distribution's hackrf package and a udev rule.
#
#  Choose the window before first use: -f (centre) and -s (sample rate) in
#  RX_ARGS below must match frequency= and sps= in config-hackrf.toml. -l / -g
#  are the LNA (0-40 dB, 8 dB steps) and VGA (0-62 dB, 2 dB steps) gains, and
#  -a 1 switches the RF amplifier on.
#
#  NOTE: not tested on HackRF hardware — it reuses the exact control/watchdog
...
RX_LABEL="HackRF One"
RX_COMM="hackrf_transfer"                          # process name to monitor/kill
CONFIG="$PHANTOMDIR/config-hackrf.toml"
FIFO="$PHANTOMDIR/hackrf.fifo"
# Keep -f / -s in step with frequency= / sps= in config-hackrf.toml.
RX_ARGS="${RX_ARGS:--r - -f 98000000 -s 20000000 -l 16 -g 16 -a 0}"
RX_CMD=(hackrf_transfer)                           # binary; args come from RX_ARGS
prestart() { :; }
# ═════════════════════════════════════════════════════════════════════════════
