...
| **QRG Sync on the receiver page** | A button on the PhantomSDR-Plus page itself, in any browser. Talks TCI to ExpertSDR, AetherSDR or Thetis, or to any Hamlib rig through a small bridge. Only on PhantomSDR-Plus receivers running 4.1.0 or later. | Frequency, mode and filter width both ways; mute on transmit |
...
Most of this manual describes Desktop PhantomSDR+. The QRG Sync button has its own section, [QRG Sync on the receiver page](#qrg-sync-on-the-receiver-page). The CATsync Tool has its own documentation on its website.
...
The button sits in the row with **VFO**, **Modes**, **Bands** and **IF Filters** — **QRG Sync** in the wide layout, **CAT** in the compact one. Its dot is **green** while a TCI server is connected and **grey** otherwise. It opens a window with:
...
|---|---|
| **SunSDR** with ExpertSDR2/3, **FlexRadio** with AetherSDR, **Apache Labs ANAN / Hermes** with Thetis | No bridge. Switch on the program's **TCI server** and use **QRG Sync** directly — ports 50001 and 40001 are found automatically |
...
The **QRG Sync** button came with 4.1.0 too, again a frontend rebuild only. It opens no port on the receiver: the connection runs from each listener's browser to their own computer. The Hamlib bridge, `tci-bridge/tci-rigctld.mjs`, is for listeners to run at home; the receiver does not use it.
...
- QRG Sync on the receiver page does not sync split, VFO B, RIT/XIT or transverter offsets, and was tried only on an IC-7300 through the Hamlib bridge.
