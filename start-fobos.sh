#!/bin/bash
# ─────────────────────────────────────────────────────────────────────────────
#  PhantomSDR-Plus  –  start-fobos.sh
#  Universal launcher + watchdog for a RigExpert Fobos SDR, RF path 25-6000 MHz (via SoapySDR rx_sdr).
#
...
#
#  Usage:
#    ./start-fobos.sh              start (or restart) the server in the background
...
#
#  Choose the RF window before first use: -f (centre) and -s (sample rate) in
#  RX_ARGS below must match frequency= and sps= in config-fobos.toml.
#
#  NOTE: not tested on Fobos hardware in this RF mode — it reuses the exact
#  control/watchdog logic validated on RX-888; only the receiver command/config
#  differ.
# ─────────────────────────────────────────────────────────────────────────────
...
RX_LABEL="Fobos SDR RF"
RX_COMM="rx_sdr"                                   # process name to monitor/kill
CONFIG="$PHANTOMDIR/config-fobos.toml"
FIFO="$PHANTOMDIR/fobos.fifo"
# Keep -f / -s in step with frequency= / sps= in config-fobos.toml.
RX_ARGS="${RX_ARGS:--d driver=fobos -f 97000000 -s 20000000 -I CF32 -F CF32 -}"
