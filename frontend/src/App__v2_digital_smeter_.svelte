...
// amkamk
  ...
  function updateTick() {
    if (events.getLastModified() > lastUpdated) {
      ...
      // Find our own pill by closest frequency match.
      // Re-detect every tick so our marker follows us as we tune.
              ...
              {/if}
                  ...
                  {#if decoderOn && radeEnabled}
                    <div class="w-full bg-gray-700 rounded-lg p-4 mt-6 text-left">
                      ...
                      <!-- FreeDV Reporter embedded iframe -->
                      {#if radeReporterOpen}
                        <div class="mt-2 rounded-lg overflow-hidden border border-gray-600" style="height:400px; background:#1f2937; overflow:hidden;">
                          <iframe
                            src="https://qso.freedv.org/"
                            title="FreeDV Reporter — Active Stations"
                            style="width:143%; height:143%; border:none; background:#1f2937; display:block; filter:invert(0) hue-rotate(0deg); transform:scale(0.70); transform-origin:top left;"
                            loading="lazy"
                            allow="fullscreen"
                          ></iframe>
                        </div>
