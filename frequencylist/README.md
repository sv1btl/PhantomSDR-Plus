...
- Unzip the file "frequencylist.zip" into the foldercd $HOME/PhantomSDR-Plus/frequencylist/.
...
- run update script with "update-markers.sh" manually or let it run automaticly by a cron job (for example update every 15 minutes a hour 'sudo crontab -e' and enter '0,15,30,45 * * * * /usr/bin/bash /home/yourLoginName/PhantomSDR-Plus-main/frequencylist/update-markers.sh')
...
- **NB** Your markers in mymarkers.json will overwrite and replace the duplicate markers from shortwavestations.json in the final markers.json file generated shortwavestations.json. The given mymarkers.json contains some demo markers, you can replace them with your own!
