#!/bin/bash
# Build ONLY the Analog version with auto title fix

set -e

echo "📦 Building Analog S-Meter Version with auto title fix"

if [ ! -f "src/App__analog_smeter_.svelte" ]; then
    echo "❌ Error: src/App__analog_smeter_.svelte not found!"
    exit 1
fi
...
cat > src/main.js << 'EOF'
import './app.css'
import App from './App__analog_smeter_.svelte'
...
echo "   Visit: http://localhost:port_used/analog/index.html"
