...
function autorunBuildMatrix(enabledSet) {
  ...
  let h = '<thead><tr><th>BAND</th>' + modes.map(m => '<th>' + m.toUpperCase() + '</th>').join('') + '</tr></thead><tbody>';
  for (const band of AR_BANDS) {
    h += '<tr><td style="color:var(--amber)">' + band + '</td>';
    for (const m of modes) {
      const key = m + ':' + band;
      if (AR_MODES[m].includes(band)) {
        h += '<td><input type="checkbox" data-key="' + key + '" onchange="autorunCount()"' + (enabledSet.has(key) ? ' checked' : '') + '></td>';
      } else {
...
async function autorunFreeAll() {
  if (!confirm('Clear all enabled bands/modes and stop the daemon?')) return;
...
def api_logs_clear():
    ...
    with log_lock:
        log_buffer.clear()
    # Truncate ALL log files defined in LOG_FILES
    base = get_sdr_dir()
    cleared = []
