...
class AudioClient : public Client {
  ...
  protected:
    ...
    // FIX: Per-instance debounce for on_demodulation_message.
    // Previously these were static locals (shared across ALL clients).
    std::mutex debounce_mutex;
    std::chrono::steady_clock::time_point debounce_last_change{
        std::chrono::steady_clock::now()};

    // FIX: Guard against double cleanup_sam between on_close() and ~AudioClient()
