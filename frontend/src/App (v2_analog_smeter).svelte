...
function _animateNeedle(ts) {
  ...
  currentPower += (targetPower - currentPower) * 0.01;
...
// ===== LED Windows (dBm & SNR) beneath the center screw =====
      // === dBm Calibration (persistent) ===
      let dBmCalOffset = 0;
      try {
        const saved = localStorage.getItem("dbmCalOffset");
        if (saved !== null && !isNaN(parseFloat(saved))) dBmCalOffset = parseFloat(saved);
      } catch (_) {}

      try { window.setDbmCalibration = setDbmCalibration; } catch (_) {}

      // Call this from console or wire to a UI input to set calibration in dB
      function setDbmCalibration(offsetDb) {
        ...
        // Optional: force a repaint tick soon after change
  ...
  function setSignalStrength(power) {
    // Clamp the power value between 0 and 100
    power = Math.min(Math.max(power, 0), 100);

    // Calibrate the power value for better S-meter response
    // Map 0-100 input to 0-100 display range with better sensitivity
    // This provides a more realistic S-meter behavior
    let calibratedPower;

    if (power < 10) {
      // Very weak signals (S0-S1): less sensitive
      calibratedPower = power * 0.3;
    } else if (power < 40) {
      // Weak to moderate signals (S1-S7): linear response
      calibratedPower = 3 + (power - 10) * 0.8;
    } else if (power < 70) {
      // Strong signals (S7-S9): moderate compression
      calibratedPower = 27 + (power - 40) * 1.0;
    } else {
      // Very strong signals (S9+): over S9 readings
      calibratedPower = 57 + (power - 70) * 1.43;
    }

    // Pass the calibrated power value to drawSMeter
  ...
  function updateTick() {
    // Get power in dB from audio
    let powerDb = audio.getPowerDb();

    // ===== Visual gain control =====
    // Increase this value to make the meter show more signal.
    // Each +0.3 roughly equals +10dB visual lift.
    const visualGain = 1.20; // correct factor S-Meter -> 1.0 = normal, 1.3 ≈ +10dB visual boost //

    // Typical S-meter calibration (HF standard)
    const minDb = -130; // S0 floor
    const s9Db = -73; // S9 reference
    const maxDb = -13; // S9 + 60dB

    // Apply visual gain by compressing the dynamic range visually
    ...
    if (powerDb < minDb) {
      ...
      // --- S0 → S9 ---
      const norm = (powerDb - minDb) / (s9Db - minDb); // 0 → 1
      const curved = Math.pow(norm, 0.6); // nonlinear boost
      power = curved * 60; // up to 60% of scale
    } else {
      // --- S9 → S9+60 ---
      const norm = (powerDb - s9Db) / (maxDb - s9Db);
      const curved = Math.pow(norm, 0.8); // softer curve for top end
      power = 60 + curved * 40; // remaining 40% of scale
    }

    // Clamp to 0–100
    power = Math.min(Math.max(power, 0), 100);

    // Light smoothing for peak - only use 3 samples instead of 10
    powerPeak = accumulator(power);

    // Update globals for dBm/SNR readouts
    try {
    ...
    // Update the S-meter display immediately
    drawSMeter(power);
    // === Helper: Convert dBm to approximate S-meter text ===
    function formatSUnits(dbm) {
      ...
      if (dbm < -79) return "S7";
      if (dbm < -73) return "S8";
      if (dbm < -63) return "S9+10";
      if (dbm < -53) return "S9+20";
      if (dbm < -43) return "S9+30";
      if (dbm < -33) return "S9+40";
      if (dbm < -23) return "S9+50";
      return "S9+60";
    }
  ...
  let mobiletopfinetuningsteps = ["-5", "-1", "-0.1", "0", "+0.1", "+1", "+5"];
  // for mobile users //
  ...
  // === dBm Visual Correction (display-only) ===
  ...
  function setDbmVisual(offsetDb) {
    ...
    try {
      ...
      requestAnimationFrame(() => drawSMeter(pwr));
    } catch (_) {}
  }
            ...
            >
              ...
              >
                ...
                <span class="text-white text-sm sm:text-sm mr-4 mb-2 sm:mb-0">
                  ...
                  ></span>-
                  <span
                    class="text-white py-1 px-3 mb-2 lg:mb-0 rounded-lg text-sm sm:text-sm"
                    style="color:rgba(0, 225, 255, 0.993)">MOBILE READY</span
                  >
                  ...
                  <button
                    ...
                    style="color:rgba(0, 225, 255, 0.993)"
                    on:click={openAudioPopup}
                    title="Opens a pop up window with audio spectrum of what you hear"
                  >
                    <span class="icon">Spectrogram</span>
                  </button>

                  &nbsp-&nbsp PhantomSDR+ WebSDR servers at:&nbsp;
                  <button
                    class="glass-button text-white py-1 px-3 mb-2 lg:mb-0 rounded-lg text-xs sm:text-sm"
                    style="color:rgba(0, 225, 255, 0.993)"
...
-->
                      ...
                      <div class="flex flex-col items-center">
                        ...
                        <script src="sMeter.js"></script>
                        <script>
                          drawSMeter(0);
                          setInterval(() => {
                            updateSMeter(Math.random() * 100);
                          }, 1000);
                        </script>
                      </div>
                    </div>
                  ...
                  <div id="frequencyContainer" class="w-full mt-4">
                    <div class="space-y-5">
                      ...
                      <div class="w-full mb-6">
                        ...
                        <div
                          id="moreoptions"
                          class="grid grid-cols-4 gap-2 text-sm h-8"
                        >
                          {#each [{ option: "NR", icon: "wave-square", enabled: NREnabled }, { option: "NB", icon: "zap", enabled: NBEnabled }, { option: "AN", icon: "shield", enabled: ANEnabled }, { option: "CTCSS", icon: "filter", enabled: CTCSSSupressEnabled }] as { option, icon, enabled }}
                            <button
