<script>
    ...
    function getClientX(e) {
      // BUG FIX (NaN / TypeError):
      // The original `if (e.clientX)` check is falsy when clientX === 0 (the
      // leftmost pixel on screen), causing the branch to fall through and attempt
      // e.detail.center.x on a plain MouseEvent — which has no .detail property
      // → TypeError: Cannot read properties of undefined.
      // When both branches failed the function returned `undefined`, making every
      // downstream computation (offset, passband drag position) NaN.
      //
      // Fix: use explicit `!== undefined` comparisons so x=0 is handled correctly,
      // and guard e.detail before accessing sub-properties.
      if (e.clientX !== undefined) {
        return e.clientX
      } else if (e.detail !== undefined && e.detail.center !== undefined && e.detail.center.x !== undefined) {
        return e.detail.center.x
      }
      return 0  // safe fallback so downstream arithmetic never produces NaN
    }
    ...
    function handleWheel(e) {
      ...
      // Fixed tuning step: one wheel notch = 100 Hz, regardless of zoom or scroll speed.
      // bandwidthToWaterfallOffset() converts a Hz delta into the matching fractional
      // passband offset for the current waterfall span, so the step stays a true 100 Hz.
      const frequencyStep = 100; // Hz
      const offsetStep = bandwidthToWaterfallOffset(frequencyStep);

      const newOffset = passbandOffset + direction * offsetStep;
  ...
  export function handleTouchEnd(e) {
    ...
    // BUG FIX: orphaned `height:` label (a stray JavaScript labeled-statement
    // that was almost certainly a typo for a CSS value reference that was never
    // removed).  It compiled as a no-op but cluttered the script.
    onMount(() => {
    })
</script>
