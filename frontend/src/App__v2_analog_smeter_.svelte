<script>
  const VERSION = "3.5.0 with mobile support and enhancements";
...
function startTopFrequencyBarSync() {
  ...
  async function _fetchClientGeo() {
    try {
      ...
      for (const u of users) {
        if (!u.id) continue;
        if (_isLocalIp(u.ip) || u.geo === 'Local') { map[u.id] = 'Local'; continue; }
    ...
    } catch (e) { /* network blip — keep previous map */ }
  }
  // --- End geo labels ---
  let bandPlanCanvas;
  let tempCanvas;
  ...
  function refreshCompMakeup() {
    if (audio && compAuto) compMakeup = Math.round(audio.getCompressorSettings().makeup * 100) / 100;
  }
  function handleCompThreshold() { if (audio) { audio.setCompressorThreshold(compThreshold); refreshCompMakeup(); } }
  function handleCompKnee()      { if (audio) { audio.setCompressorKnee(compKnee); refreshCompMakeup(); } }
  function handleCompRatio()     { if (audio) { audio.setCompressorRatio(compRatio); refreshCompMakeup(); } }
  function handleCompAttack()    { if (audio) audio.setCompressorAttack(compAttack); }
  function handleCompRelease()   { if (audio) audio.setCompressorRelease(compRelease); }
  function handleCompMakeup()    { if (audio && !compAuto) audio.setCompressorMakeup(compMakeup); }
  ...
  function handleCompReset() {
    compThreshold = -24; compKnee = 6; compRatio = 4;
    compAttack = 0.003; compRelease = 0.25; compAuto = true;
    if (audio) {
  ...
  onMount(() => {
    console.log('[WF-MSG] Admin message polling active — fetching /wf-message.json');
  ...
  function videoRecordingCaption() {
    ...
    return (Number(frequency) || 0).toFixed(2) + " kHz  " + demodulation + "  " + utc + "Z";
  }
  ...
  try {
    ...
    const hideHighlights = () => {
      ...
      if (spectrumHighlightInner) spectrumHighlightInner.style.display = "none";
    };
    ...
    if (spectrumHighlightInner) {
      if (spectrumDisplay) {
        ...
        spectrumHighlightInner.style.borderLeft = "2px solid rgba(255,255,200,0.20)";
        spectrumHighlightInner.style.borderRight = "2px solid rgba(255,255,200,0.20)";
...
// Window for shortcuts
  ...
  function handleFtxAutoSync() { audio?.setFTxAutoSync?.(ftxAutoSync); }
  // Let the auto-sync loop drive the slider, and adopt the stored value on load.
  function bindFtxSync() {
    if (!audio) return;
    if (typeof audio.getFTxTimeShift === 'function') ftxTimeShift = audio.getFTxTimeShift();
    if (typeof audio.ftxAutoSync === 'boolean') ftxAutoSync = audio.ftxAutoSync;
    audio.onFTxTimeShiftChange = (v) => { ftxTimeShift = v; };
  }
  ...
  function _wsprTickStart() {
    ...
    _wsprTimer = setInterval(() => {
      ...
      wsprPhase = wsprSlotPos >= 119 ? 'decoding'
  ...
  const FAX_STATIONS = [
    ...
    { name: 'XSQ — China Guangzhou',           freqs: [4199.75, 8412.5, 12629.25, 16826.25],               lpm: 120, ioc: 576 },
    // ── Pacific ───────────────────────────────────────────────────────────
    ...
    { name: 'NOJ — USA Kodiak AK',             freqs: [2054,   4298,   8459,   12412.5],                         lpm: 120, ioc: 576 },
  ...
  const FAX_SCHEDULE = {
    ...
    'NMG — USA New Orleans LA':        [ {hh:0,mm:26,label:'Surface Analysis (18Z) Gulf/Caribbean'},{hh:1,mm:25,label:'24hr Forecast Gulf/Caribbean'},{hh:3,mm:25,label:'Surface Analysis (00Z) Gulf/Caribbean'},{hh:6,mm:25,label:'Surface Analysis (06Z) Gulf/Caribbean'},{hh:7,mm:25,label:'24hr Forecast Gulf/Caribbean'},{hh:12,mm:25,label:'Surface Analysis (12Z) Gulf/Caribbean'},{hh:13,mm:25,label:'24hr Forecast Gulf/Caribbean'},{hh:18,mm:25,label:'Surface Analysis (18Z) Gulf/Caribbean'},{hh:19,mm:25,label:'24hr Forecast Gulf/Caribbean'} ],
  ...
  function startStatusMonitoring() {
    ...
    statusInterval = setInterval(() => {
      if (autoAdjustEnabled) {
        ...
        if (Number.isFinite(waterfall.autoAdjustDisplayMin)) min_waterfall = Math.round(waterfall.autoAdjustDisplayMin);
        if (Number.isFinite(waterfall.autoAdjustDisplayMax)) max_waterfall = Math.round(waterfall.autoAdjustDisplayMax);
      }
  ...
  function handleDemodulationChange(e, changed) {
    ...
    if (changed) {
      ...
      audio.setAudioDemodulation(
        demodulation === "AM" && !samEnabled ? "AM-ENV" : demodulationDefault.type,
      );
    }
  ...
  function _deactivateAll() {
    ...
    const _ftxWasActive = ft8Enabled || ft4Enabled || ft2Enabled || wsprEnabled;
    // FT8 / FT4
    if (ft8Enabled) { ft8Enabled = false; audio.setFT8Decoding(false); }
    if (ft4Enabled) { ft4Enabled = false; audio.setFT4Decoding(false); }
    if (ft2Enabled) { ft2Enabled = false; audio.setFT2Decoding(false); }
    // CW
  ...
  function handleFt2Decoder(e, value)   { selectedDecoder = 'ft2';  decoderOn = !!value; if (decoderOn) activateSelectedDecoder(); else _deactivateAll(); }
...
function handlePassbandChange(passband) {
  ...
  if (!isWheel) {
    ...
    let clickedFreq;
    if (blockFrac < 355)      clickedFreq = blockBase;         // .00 (this kHz)
    else if (blockFrac > 645) clickedFreq = blockBase + 1000;  // .00 (next kHz)
...
// --- S-Meter face theme -----------------------------------------------------
// 'amber' = vintage illuminated face; 'dark' = original dark brushed metal.
// Click the meter to toggle; the choice persists across sessions.
let smeterTheme = 'dark';
// One-time reset: bump SMETER_PREF_VERSION to make every browser forget a
// previously-saved *amber* choice ONCE, so the current dark default takes over.
// The reset runs only while the stored version is behind; afterwards the toggle
// persists normally again (re-selecting amber sticks). Bump again for future resets.
const SMETER_PREF_VERSION = 2;
try {
  const ver = parseInt(localStorage.getItem('smeterThemePrefV') || '0', 10);
  if (!(ver >= SMETER_PREF_VERSION)) {
    if (localStorage.getItem('smeterTheme') === 'amber') {
  ...
  if (_st === 'amber' || _st === 'dark') smeterTheme = _st;
} catch (e) {}

function toggleSMeterTheme() {
  smeterTheme = smeterTheme === 'amber' ? 'dark' : 'amber';
...
function _animateNeedle(ts) {
  ...
  function drawSMeterDesktop(powerValue) {
    ...
    const amber = smeterTheme === 'amber';

    if (amber) {
      // ===== Warm Amber Illuminated Face =====
...
// ===== LED Windows (dBm & SNR) beneath the center screw =====
  ...
  function handleNBChange() {
    ...
    if (audio) {
      ...
      if (typeof audio.setHumNotch === "function") {
        audio.setHumNotch("both", NBEnabled);
      }
    }
  }
  ...
  function _smeterTick() {
    ...
    try {
      ...
      const _wf = (typeof waterfall.getSnrEstimate === "function") ? waterfall.getSnrEstimate() : null;
  ...
  function updateTick() {
    ...
    { const _q = (audio && typeof audio.getCquamPilotDetected === "function") ? audio.getCquamPilotDetected() : false; if (_q !== cquamPilotDetected) cquamPilotDetected = _q; }
    { const _s = (audio && typeof audio.getSamLocked === "function") ? audio.getSamLocked() : false; if (_s !== samLocked) samLocked = _s; }
    if (events.getLastModified() > lastUpdated) {
      ...
      let myId = null;
      const realId = (typeof audio.getClientId === "function") ? audio.getClientId() : null;
  ...
  function detectMode(info) {
    if (!info) return '';
    const modes = ['FT8','FT4','FT2','CW','SSB','AM','FM','RTTY','PSK31','BPSK31','JS8','WSPR','JT65','JT9','MSK144'];
      ...
      >
        ...
        >
          <div class="max-w-screen-lg mx-auto">
            ...
            >
              <div
                class="p-3 sm:p-5 flex flex-col bg-gray-800 border border-gray-700 rounded-lg w-full mb-8 screw-panel"
                id="chat-box"
                style="opacity: 0.85;"
              >
                <!-- Header -->
                ...
                <div class="columns">
                  <div class="first-column">
                    ...
                    <b>Setup &amp; Configuration:</b>
                      <img
                        src="https://img.shields.io/badge/version- 3.5.0-cyan?logo=github"
                        alt="Version"
              ...
              <div class="flex justify-center w-full">
                <div class="w-full" id="outer-waterfall-container">
                  ...
                  >
                    ...
                    {#if adminMessage}
                      <div style="width:100%;background:rgba(0,0,0,0.92);
                                  border-left:4px solid #ffb000;padding:6px 14px;
                                  font-family:monospace;font-size:13px;font-weight:bold;
                                  display:flex;align-items:center;gap:10px;
                                  box-shadow:0 2px 6px rgba(0,0,0,0.5);">
                        <span style="color:#ffb000;flex-shrink:0;">&#9733; ADMIN:</span>
                        <span style="color:{adminMsgColor};overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">{adminMessage}</span>
                      </div>
                    {/if}
                    <div class="relative w-full">
                      <canvas
              ...
              <!-- Begin Band Selection -->
                  ...
                  >
                    {#each ["USB", "LSB", "CW", "AM", "QUAM", "FM"] as mode}
                      <button
                        on:click={() => SetMode(mode, true)}
                        class="retro-button text-sm {mode === 'QUAM' && cquamPilotDetected ? 'text-green-400 font-bold' : mode === 'AM' && samLocked ? 'text-yellow-400 font-bold' : 'text-white'} fontrbold h-7 text-base rounded-md flex items-center justify-center border border-gray-600 shadow-inner transition-all duration-200 ease-in-out {demodulation ===
                        mode
                          ? 'bg-blue-600 pressed scale-95'
                          : 'bg-gray-700 hover:bg-gray-600'}"
                      >
                        {mode === 'AM' && demodulation === 'AM' && samEnabled ? 'SAM' : mode}
                      </button>
                    {/each}
                  </div>
                  <!-- End of Mode Content -->
...
Click again to de-activate"
                    ...
                    >
                      ...
                      >
                        <div class="flex justify-between items-center mb-4">
                          <h2 class="text-base font-bold text-white">Equalizer</h2>
                          <button
                        ...
                        <div class="flex items-center justify-between mb-4">
                          <span class="text-white text-sm font-semibold">Enable Equalizer</span>
                          <button
                        ...
                        <div class="flex flex-col gap-3">
                          {#each eqBandLabels as label, i}
                            <div class="flex items-center gap-3">
                              <span class="text-gray-300 text-xs w-14 text-right">{label}</span>
                              ...
                              <span class="text-gray-300 text-xs w-12 text-right">{eqGains[i]} dB</span>
                            </div>
                          {/each}
                        </div>
                  ...
                  {#if showCompPopup}
                    ...
                    >
                      <div
                        class="bg-gray-800 p-6 rounded-lg max-w-lg w-full max-h-[80vh] flex flex-col overflow-y-auto"
                        on:click|stopPropagation
                      >
                        <div class="flex justify-between items-center mb-4">
                          <h2 class="text-base font-bold text-white">Compressor</h2>
                          <button
                            class="text-gray-400 hover:text-white"
                            on:click={toggleCompPopup}
                          >
                        ...
                        <!-- Enable toggle -->
                        <div class="flex items-center justify-between mb-4">
                          <span class="text-white text-sm font-semibold">Enable Compressor</span>
                          <button
                            class={`retro-button text-white font-bold h-8 px-4 text-sm rounded-md flex items-center justify-center border border-gray-600 shadow-inner transition-all duration-200 ease-in-out ${
                              compressorEnabled
                                ? 'bg-blue-600 pressed scale-95'
                        ...
                        <div class="flex flex-col gap-3">
                          <div class="flex items-center gap-3">
                            <span class="text-gray-300 text-xs w-20 text-right">Threshold</span>
                            <div class="slider-container flex-1 mx-0">
                              <input type="range" min="-60" max="0" step="1"
                                bind:value={compThreshold} on:input={handleCompThreshold}
                                title="Threshold" class="glass-slider" />
                            </div>
                            <span class="text-gray-300 text-xs w-16 text-right">{compThreshold} dB</span>
                          </div>

                          <div class="flex items-center gap-3">
                            <span class="text-gray-300 text-xs w-20 text-right">Ratio</span>
                            <div class="slider-container flex-1 mx-0">
                              <input type="range" min="1" max="20" step="0.5"
                                bind:value={compRatio} on:input={handleCompRatio}
                                title="Ratio" class="glass-slider" />
                            </div>
                            <span class="text-gray-300 text-xs w-16 text-right">{compRatio}:1</span>
                          </div>

                          <div class="flex items-center gap-3">
                            <span class="text-gray-300 text-xs w-20 text-right">Attack</span>
                            <div class="slider-container flex-1 mx-0">
                              <input type="range" min="0" max="0.1" step="0.001"
                                bind:value={compAttack} on:input={handleCompAttack}
                                title="Attack" class="glass-slider" />
                            </div>
                            <span class="text-gray-300 text-xs w-16 text-right">{Math.round(compAttack * 1000)} ms</span>
                          </div>

                          <div class="flex items-center gap-3">
                            <span class="text-gray-300 text-xs w-20 text-right">Release</span>
                            <div class="slider-container flex-1 mx-0">
                              <input type="range" min="0.01" max="1" step="0.01"
                                bind:value={compRelease} on:input={handleCompRelease}
                                title="Release" class="glass-slider" />
                            </div>
                            <span class="text-gray-300 text-xs w-16 text-right">{Math.round(compRelease * 1000)} ms</span>
                          </div>

                          <div class="flex items-center gap-3">
                            <span class="text-gray-300 text-xs w-20 text-right">Knee</span>
                            <div class="slider-container flex-1 mx-0">
                              <input type="range" min="0" max="40" step="1"
                                bind:value={compKnee} on:input={handleCompKnee}
                                title="Knee" class="glass-slider" />
                            </div>
                            <span class="text-gray-300 text-xs w-16 text-right">{compKnee} dB</span>
                          </div>

                          <div class="flex items-center gap-3">
                            <span class="text-gray-300 text-xs w-20 text-right">Makeup</span>
                            <div class="slider-container flex-1 mx-0">
                              <input type="range" min="0" max="4" step="0.1"
                                bind:value={compMakeup} on:input={handleCompMakeup}
                                disabled={compAuto}
                                title="Makeup gain" class="glass-slider" />
                            </div>
                            <span class="text-gray-300 text-xs w-16 text-right">{compMakeup}×</span>
                          </div>
                          <div class="flex items-center gap-3">
                            <span class="text-gray-300 text-xs w-20 text-right">Auto</span>
                            <label class="flex items-center gap-2 flex-1 cursor-pointer">
                              <input type="checkbox" bind:checked={compAuto} on:change={handleCompAutoToggle} />
                              <span class="text-gray-300 text-xs">Auto makeup (tracks threshold &amp; ratio)</span>
                            </label>
                            <span class="text-gray-300 text-xs w-16 text-right"></span>
                          </div>
                        </div>
                        ...
                        >
                          ...
                          >
                            ...
                            {:else if icon === "zap"}
                              ...
                              />
                            {:else if icon === "waves"}
                              <path d="M2 6c1.5-2 3.5-2 5 0s3.5 2 5 0 3.5-2 5 0 3.5 2 5 0" />
...
-->
                        ...
                        >
                          <div
                            class="px-1 py-0.5 flex items-center justify-center w-8 h-5 relative overflow-hidden"
                          >
                            <span
                      ...
                      <div class="flex flex-col items-center">
                        <!-- SMeter (click to switch face: vintage amber / dark) -->
                        <div
                          class="smeter-canvas-wrap {smeterTheme === 'amber'
                            ? 'theme-amber'
                            : 'theme-dark'}"
                        >
                          <canvas
                            ...
                            role="button"
                            tabindex="0"
                            title="Click to switch meter style (vintage / dark)"
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  {#each ["USB", "LSB", "CW", "AM", "QUAM", "FM"] as mode}
                                    <button
                                      class="retro-button {mode === 'QUAM' && cquamPilotDetected ? 'text-green-400 font-bold' : mode === 'AM' && samLocked ? 'text-yellow-400 font-bold' : 'text-white'} font-bold h-10 text-base rounded-md flex items-center justify-center border border-gray-600 shadow-inner transition-all duration-200 ease-in-out {demodulation ===
                                      mode
                                        ? 'bg-blue-600 pressed scale-95'
                                        : 'bg-gray-700 hover:bg-gray-600'}"
                            ...
                            >
                              ...
                              >
                                ...
                                <div class="w-full mt-4">
                                  ...
                                  >
                                    {#each newBandwidth as newbandwidth}
                                      <button
                                        ...
                                        class="retro-button text-sm text-white font-bold h-8 text-lg rounded-md flex items-center justify-center border border-gray-600 shadow-inner transition-all duration-200 ease-in-out {newStaticBandwidth == newbandwidth || Math.round(bandwidth * 1000) ==
                                        newbandwidth
                                          ? 'bg-blue-600 pressed scale-95'
                                          : 'bg-gray-700 hover:bg-gray-600'}"
                                        on:click={() =>
                                  ...
                                  <!-- IF Filter bandwidth slider -->
                                  <div class="w-full mt-4 border-t border-gray-600 pt-3">
                                    <div class="flex items-center justify-between mb-1">
                                      <label class="text-gray-400 text-xs">Bandwidth (center &#8594; edge)</label>
                                      <span class="value-display text-gray-300 text-xs whitespace-nowrap">{ifSlider} Hz</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                      <div class="slider-container flex-1">
                                        <input type="range" class="glass-slider" bind:value={ifSlider} on:input={handleIFSlider} min="0" max={ifSliderMax} step="100" title="Adjust bandwidth from centre toward the active sideband" />
                                      </div>
                                      <button class="retro-button text-xs text-white font-bold h-8 px-3 rounded-md flex items-center justify-center border border-gray-600 shadow-inner bg-gray-700 hover:bg-gray-600 transition-all duration-200 ease-in-out whitespace-nowrap" on:click={resetIFFilter} title="Reset passband to mode default">Reset</button>
                                    </div>
                                  </div>
                                <!-- Content Ends -->
                              </div>
                  ...
                  {#if decoderOn && (ft8Enabled || ft4Enabled || ft2Enabled)}
                    <div class="w-full bg-gray-700 rounded-lg p-6 mt-6">
                      <div class="w-full flex justify-between items-center mb-5 text-xs">
                        <h4 class="text-white font-semibold">{ft2Enabled ? 'FT2' : ft4Enabled ? 'FT4' : 'FT8'} Messages</h4>
                      ...
                      <div class="flex items-center gap-2 text-xs text-gray-300 mb-1">
                        <label class="whitespace-nowrap" title="Capture lead-in after the UTC slot boundary, to offset audio pipeline latency (KiwiSDR time_shift)">Sync offset</label>
                        <input type="range" min="0" max="3" step="0.05"
                               bind:value={ftxTimeShift} on:input={handleFtxTimeShift}
                               class="flex-grow accent-cyan-400" />
                        <span class="font-mono w-12 text-right">{ftxTimeShift.toFixed(2)}s</span>
                        <label class="flex items-center gap-1 whitespace-nowrap" title="Track the capture timing automatically from the decoders' own DT measurement">
                          <input type="checkbox" bind:checked={ftxAutoSync} on:change={handleFtxAutoSync} class="accent-cyan-400" />
                          Auto
                        </label>
                      </div>
                        ...
                        <div class="w-full mt-1 mb-2 h-2 rounded-full bg-gray-600 overflow-hidden" title="WSPR slot progress">
                          {#if wsprPhase === 'collecting'}
                            <div class="h-full rounded-full bg-cyan-400 transition-all duration-500"
                                 style="width:{(wsprSlotPos / 119 * 100).toFixed(1)}%"></div>
                  ...
                  {#if decoderOn && sstvEnabled}
                    <div class="mt-3 rounded-xl border border-cyan-500/25 bg-black/35 p-3 shadow-inner">
                      ...
                      <div class="flex flex-wrap items-center gap-3 mb-2 text-xs text-gray-300">
                        ...
                        <span class="text-gray-400">Lines: <span class="text-gray-200">{sstvLineCount}</span>/{sstvCanvasH}</span>
                      </div>
                          ...
                          <div class="overflow-y-auto max-h-40 custom-scrollbar divide-y divide-gray-800">
                            {#each navtexScheduleRows as row, i}
                              <div class="flex items-center gap-2 px-2.5 py-1 {i === 0 ? 'bg-gray-800/70' : 'hover:bg-gray-800/40'} transition-colors">
                                <span class="shrink-0 text-sm leading-none">{@html window.twemoji ? window.twemoji.parse(row.flag) : row.flag}</span>
                  ...
                  {#if decoderOn && radeEnabled}
                    <div class="w-full bg-gray-700 rounded-lg p-4 mt-6 text-left">
                      ...
                      <div class="mt-3 flex items-center justify-end gap-2">
                        <button
                          class="text-xs px-3 py-1 rounded {radeReporterOpen ? 'bg-green-500 hover:bg-green-400' : 'bg-green-700 hover:bg-green-600'} text-white transition-colors whitespace-nowrap"
                          on:click={() => radeReporterOpen = !radeReporterOpen}
                        >
                          {radeReporterOpen ? '▲ Hide Reporter' : '📡 FreeDV Reporter'}
                        </button>
                        {#if radeReporterOpen}
                          <a
                      ...
                      {#if radeReporterOpen}
                        ...
                        <p class="text-gray-600 text-xs mt-1 text-right">
                          Live · <a href="https://qso.freedv.org/" target="_blank" rel="noopener noreferrer" class="hover:text-gray-400 transition-colors">qso.freedv.org</a>
                        </p>
                      {/if}
                    </div>
                  {/if}
...
<!-- Begin Bookmark Button Area -->
                  ...
                  <div class="mt-6 w-full">
                    ...
                    <div class="flex flex-nowrap justify-center gap-2">
                      ...
                      >
                        {selectingArea ? "Cancel" : videoCrop ? "Area \u2713" : "Area"}
                      </button>
                    </div>
              ...
              <div class="flex justify-center w-full">
                <div class="w-full" id="outer-waterfall-container">
                  ...
                  >
                    ...
                    {#if adminMessage}
                      <div style="width:100%;background:rgba(0,0,0,0.88);
...
<style global lang="postcss">
  ...
  * {
  ...
  /* Clickable S-meter affordance: corner glyph + hover hint */
  .smeter-canvas-wrap {
    position: relative;
    display: inline-block;
    line-height: 0;
  ...
  .ftx-grid {
    display: grid;
    grid-template-columns: 2.6rem 2.6rem 3.4rem 3rem minmax(0, 1fr) 4.6rem 4.6rem;
    gap: 0.5rem;
    align-items: center;
...
/*Hide Input Number Arrows*/
  ...
  .screw-panel {
    position: relative;
    --screw-head: radial-gradient(circle closest-side at 50% 50%,
      #ededee 0%, #d0d1d4 26%, #a6a7ad 50%, #77787f 72%,
      #4f5057 87%, #313137 96%, rgba(18, 18, 22, 0.92) 99%, transparent 100%);
    --screw-spec: radial-gradient(circle closest-side at 38% 34%,
      rgba(255, 255, 255, 0.75) 0%, rgba(255, 255, 255, 0.12) 34%,
      rgba(255, 255, 255, 0) 55%);
    /* one straight slot per corner, each at its own "hand-driven" angle */
    --screw-slot-tl: linear-gradient(27deg,
      transparent 41%, rgba(255, 255, 255, 0.16) 43%, rgba(15, 15, 18, 0.80) 45%,
      rgba(15, 15, 18, 0.80) 55%, rgba(255, 255, 255, 0.08) 57%, transparent 59%);
    --screw-slot-tr: linear-gradient(112deg,
      transparent 41%, rgba(255, 255, 255, 0.16) 43%, rgba(15, 15, 18, 0.80) 45%,
      rgba(15, 15, 18, 0.80) 55%, rgba(255, 255, 255, 0.08) 57%, transparent 59%);
    --screw-slot-bl: linear-gradient(66deg,
      transparent 41%, rgba(255, 255, 255, 0.16) 43%, rgba(15, 15, 18, 0.80) 45%,
      rgba(15, 15, 18, 0.80) 55%, rgba(255, 255, 255, 0.08) 57%, transparent 59%);
    --screw-slot-br: linear-gradient(143deg,
      transparent 41%, rgba(255, 255, 255, 0.16) 43%, rgba(15, 15, 18, 0.80) 45%,
      rgba(15, 15, 18, 0.80) 55%, rgba(255, 255, 255, 0.08) 57%, transparent 59%);
  }
  .screw-panel::after {
    ...
    background-size:
      11px 11px, 15px 15px, 15px 15px,
      11px 11px, 15px 15px, 15px 15px,
      11px 11px, 15px 15px, 15px 15px,
      11px 11px, 15px 15px, 15px 15px;
    background-position:
      left 10px top 10px, left 8px top 8px, left 8px top 8px,
      right 10px top 10px, right 8px top 8px, right 8px top 8px,
      left 10px bottom 10px, left 8px bottom 8px, left 8px bottom 8px,
      right 10px bottom 10px, right 8px bottom 8px, right 8px bottom 8px;
  }
