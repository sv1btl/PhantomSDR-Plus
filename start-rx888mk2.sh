...
#
#  Usage:
#    ./start-rx888mk2.sh            start (or restart); mirrors the log to the
#                                   terminal until the server is confirmed up
#    ./start-rx888mk2.sh -q         same, but only two lines of output
...
# ── derive an optional CPU pin for spectrumserver + rx888_stream ─────────────
# On boxes with >4 cores, pin the FFT/audio threads and the USB sample reader to
# the lower cores and keep the top few free (reduces waterfall/audio jitter).
# The reserved top cores are where the autorun daemon pins its decode workers,
# and a decode burst can saturate them all — which is exactly what the sample
# reader must not be sharing. Not heat-neutral for the reader: it moves work onto
# higher-clocked cores. Fully generic via nproc; SPECTRUM_CORES overrides (a list
# to pin, or none/off to disable both pins).
compute_taskset() {
    SS_TASKSET=()
...
start_rx888() {
    ...
    for attempt in 1 2 3; do
        ...
        mkfifo "$FIFO"
        exec 8<>"$FIFO"
        # taskset execs into the target, so $! stays the real rx888_stream PID —
        # the kill -0 check below and killall -KILL rx888_stream are unaffected.
        # shellcheck disable=SC2086
        "${SS_TASKSET[@]}" "$RX888" $RX888_ARGS > "$FIFO" &
        RX_PID=$!
        sleep 5
...
#
# The watchdog itself is detached from the terminal (that is the whole point —
# it must outlive the shell), so it can only write to $LOG. To still show what
# is going on, the launcher mirrors the log to the terminal while the bring-up
# runs, then reports the outcome and exits. Killing the launcher (Ctrl-C) only
# stops the mirroring; the watchdog keeps going. -q restores the old two-line
# output for scripted/unattended use.
launch() {
    local quiet="$1" tail_pid="" up=0 i

    echo "PhantomSDR-Plus (RX-888 MkII)"
    ...
    # Watch for the outcome. Worst case is ~30s of rx888 USB retries plus the
    # ~120s confirm poll inside bring_up, so give it a little over 3 minutes
    # before handing back the prompt — the watchdog carries on either way.
    for i in $(seq 1 65); do
        sleep 3
        if is_running spectrumserver && is_running rx888_stream; then up=1; break; fi
    done
    ...
    if [ "$up" = 1 ]; then
        echo "  ✔ server is up   (spectrumserver + rx888_stream running)"
        if ! rade_wanted; then
            echo "  · $(rade_reason)"
