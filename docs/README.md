...
### S-meter calibration
Although the smeter is already calibrated, if you wish to re-adjust it, do the following:
- For **Digital smeter** you have to adjust it in used .toml file. Find 'smeter_offset=' in [input] section, adjust it and restart the server.
- For **Analog smeter** you will need to edit 'App__analog_smeter_.svelte' and 'App__v2_analog_smeter_.svelte' both located in frontend/src. Search for 'const visualGain = ' and change the value so digital and analog smeter to show the same. Then please recompile. There is no need to restart the server.
