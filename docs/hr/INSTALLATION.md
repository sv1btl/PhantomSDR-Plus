...
Svaki je dodatak obična skripta koju možete pokrenuti i zasebno, kad god želite — `./setup_admin.sh` ([priručnik](ADMIN_PANEL_SETUP.md)), `./install_rade.sh` ([priručnik](RADE_README.md)), `./install-stats-server.sh` ([priručnik](sdr-stats/README.md)) i `./setup-rx888-udev.sh` — upravo njih poziva instalacijska skripta.
