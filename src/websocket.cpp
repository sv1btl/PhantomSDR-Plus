...
void broadcast_server::init_server() {
    ...
    // Suppress all error logs - we'll handle them ourselves
    m_server.clear_error_channels(websocketpp::log::elevel::all);
    
    // Only log fatal errors (suppress info, warn, rerror, devel)
    // This suppresses EOF errors and other transport errors
    m_server.set_error_channels(websocketpp::log::elevel::fatal);
    
    // Set custom error handler to catch transport errors
    m_server.set_fail_handler([this](connection_hdl hdl) {
        // Silently handle failed connections
        try {
            auto con = m_server.get_con_from_hdl(hdl);
            auto ec = con->get_ec();
            
            // Only log if it's NOT an EOF or transport error
