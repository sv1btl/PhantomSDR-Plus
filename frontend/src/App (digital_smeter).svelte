<script>
  const VERSION = "1.6.0 with mobile support and enhancements";
  ...
  import { siteSysop, siteSysopEmailAddress, siteInformation, siteGridSquare, siteCity, siteHardware, siteSoftware, siteReceiver, siteAntenna, siteNote, siteIP,  siteSDRBaseFrequency, siteSDRBandwidth, siteRegion, siteChatEnabled,  } from '../site_information.json';
...
let currentAGC = 0;
  ...
  function updateTime() {
    ...
    let minutes = Digital.getUTCMinutes();
		let seconds = Digital.getUTCMinutes();
 ...
 // Declaration for the VFO A/B system //
  ...
  function handleAutoAdjust() {
    ...
    if (autoAdjustEnabled) {
      ...
      // If the renderer supports an explicit auto-adjust trigger, call it
      if (typeof waterfall.enableAutoAdjust === "function") {
        waterfall.enableAutoAdjust();
      }
    } else {
...
// DOWNLOADLOAD BOOKMARKS
  ...
  function downloadBookmarks() {
                ...
                //
                const bookmarkFullDate = bookmarkYear + '-' + bookmarkDay + '-' + bookmarkMonth;
                const bookmarkTime = bookmarkHour + '-' + bookmarkMinute + '-' + bookmarkSeconds;
                const timeStamp = bookmarkFullDate + '_' + bookmarkTime;
                // Create download link
                const url = URL.createObjectURL(new Blob([jsonFile], { type: 'JSON/json' }));
          ...
          <div class="flex flex-col rounded p-2 justify-center" id="chat-column">
            <div class="p-3 sm:p-5 flex flex-col bg-gray-800 border border-gray-700 rounded-lg w-full mb-8" id="chat-box" style="opacity: 0.85;">
              ...
              <h4 class="text-xl sm:text-2xl font-semibold text-gray-100 mb-2 sm:mb-4">
                  WebSDR <a href="https://www.qrz.com/db/{siteSysop}" target="new" style="color:rgba(0, 225, 255, 0.993)">{siteSysop}</a>, located in <a href="http://k7fry.com/grid/?qth={siteGridSquare}" target="new" style="color:rgba(0, 225, 255, 0.993)">{siteCity}, {siteGridSquare}</a>             
              </h4>

              <!-- Details -->
                  <span class="text-white text-sm sm:text-sm mr-4 mb-2 sm:mb-0">
                  {siteSysop}:
                  <a href="mailto:{siteSysopEmailAddress}?subject=WebSDR" style="color:rgba(0, 225, 255, 0.993)">email</a>
                  <span class="text-white text-xs sm:text-sm mr-4 mb-2 sm:mb-0"></span>-
                  <span class="text-white py-1 px-3 mb-2 lg:mb-0 rounded-lg text-sm sm:text-sm" style="color:rgba(0, 225, 255, 0.993)">MOBILE READY</span>
              ...
              <script>
                ...
                function openAudioPopup() {
                  popup = window.open(
                    'https://psychomed.gr/audio/indexalt.html', // URL to open
                    'AudioPopup', // popup window name
                    'width=800,height=400,left=200,top=100,location=no,toolbar=no,menubar=no,resizable=yes,scrollbars=yes'
                  );
                  ...
                  // Focus popup when the parent window regains focus
              ...
              <button
                ...
                on:click={openAudioPopup} title="Opens a pop up window with audio spectrum of what you hear">
                <span class="icon">Spectrogram</span>
              </button>

                    
                    &nbsp-&nbsp
                  PhantomSDR+ WebSDR servers at:&nbsp;
                  <button class="glass-button text-white py-1 px-3 mb-2 lg:mb-0 rounded-lg text-xs sm:text-sm" style="color:rgba(0, 225, 255, 0.993)" title="Other servers lookup" onClick="window.open('https://sdr-list.xyz/');">
              ...
              <!-- Floating popup show shortcuts -->
                <button
                  class="shortcuts-side-btn"
              ...
              Frequency Lookup :&nbsp; 
                              <button class="glass-button text-white py-1 px-3 mb-2 lg:mb-0 rounded-lg text-sm sm:text-sm" style="color:rgba(0, 225, 255, 0.993)" title="Find the MW you are hearing" onClick="window.open('http://www.mwlist.org/mwlist_quick_and_easy.php?area=1&amp;kHz='+{Math.round(frequency)},'websdrstationinfo','');">
                                <span class="icon">MW List</span>
                                </button>
              &nbsp;&nbsp;&nbsp;
                              <button class="glass-button text-white py-1 px-3 mb-2 lg:mb-0 rounded-lg text-sm sm:text-sm" style="color:rgba(0, 225, 255, 0.993)" title="Find the SW you are hearing" onClick="window.open('http://www.short-wave.info/index.php?freq='+{Math.round(frequency)}+'&amp;timbus=NOW&amp;ip=179&amp;porm=4','websdrstationinfo','')">
              ...
              <form method="get" target="_blank" action="https://www.qrzcq.com" >
                              ...
                              <input class="glass-button text-white py-1 px-3 mb-2 lg:mb-0 rounded-lg text-xs sm:text-sm" type="submit" name="page" value="Search" style="color:rgba(0, 225, 255, 0.993)" />
                              </form>
                <br>
              </div>
               ...
               <div id="collapsible-menu" class="hidden mt-3 bg-gray-700 p-3 rounded">
                <div class="columns">
                  <div class="first-column">
                    ...
                    <br>                    
                    <span style="/*text-decoration: line-through*/">Antenna: {siteAntenna}</span> <br><br>
...
Shift + Ctrl + Wheel = snap to .00 KHz"
          ...
          <!-- Audio Begins -->
            ...
            <div><hr class="border-gray-600 my-2" /></div>
              ...
              <h3 class="text-white text-base font-semibold mb-2">Filters</h3>
              <div class="w-full mb-6">
                <div id="moreoptions" class="grid grid-cols-4 gap-2">
                ...
                <hr class="border-gray-600 m-auto" />
             ...
             <div><hr class="border-gray-600 my-2" /></div>
              ...
              <!-- Begin Modes Selection -->
               <h3 class="text-white text-base font-semibold mb-2">Modes selector</h3>            
                    <div id="demodulationModes" class="grid grid-cols-3 sm:grid-cols-5 gap-2 w-full max-w-md">
                      {#each ['USB', 'LSB', 'CW', 'AM', 'FM'] as mode}
                        <button
                          ...
                          class="retro-button text-sm text-white fontrbold h-7 text-base rounded-md flex items-center justify-center border border-gray-600 shadow-inner transition-all duration-200 ease-in-out {demodulation === mode ? 'bg-blue-600 pressed scale-95' : 'bg-gray-700 hover:bg-gray-600'}"
                        >
                          {mode}
                        </button>
                              {/each}
...
-->
             ...
             <!-- Decoder Options -->
                <h3 class="text-white text-base font-semibold mb-2">Decoder Options</h3>
...
<!-- upload bookmark section -->
                    ...
                    <div class="mb-6">
                      <label
                        class="block text-sm font-medium text-gray-300 mb-2"
                        >Upload - Download Bookmarks. If you don't see them, please refresh the webpage (or press F5)! </label
                      >
                      <div class="flex items-center gap-2">
          ...
          >
            ...
            >
              ...
              <div class="mb-2 sm:mb-4 flex flex-wrap items-center">
                <span class="text-white text-xs sm:text-sm mr-2 mb-2 sm:mb-0"
                  >Chatting as:</span
                >
    ...
    <div class="w-full sm:h-screen overflow-y-scroll sm:w-1/2 xl:w-1/3 lg:w-1/4 sm:transition-all sm:ease-linear sm:duration-100" style="width:100%;">
      <div class="min-h-screen bg-custom-dark text-gray-200" style="padding-top: 5px;">
        <div class="max-w-screen-lg mx-auto">
          ...
          <!--Titel Box with Admin Infos, to be personalized-->
          <div class="flex flex-col rounded p-2 justify-center" id="chat-column">
            <div class="p-3 sm:p-5 flex flex-col bg-gray-800  rounded-lg w-full mb-8" id="chat-box" style="opacity: 0.85;">
              <!-- Header -->
              <div class="text-xs sm:text-sm font-semibold text-gray-100 mb-2 sm:mb-4">
                 <a href="https://www.qrz.com/db/{siteSysop}" target="new"
                style="color:rgba(0, 225, 255, 0.993)">{siteSysop}</a> PhantomSDR+ in <a href="http://k7fry.com/grid/?qth={siteGridSquare}"
                target="new" style="color:rgba(0, 225, 255, 0.993)">{siteCity}, {siteGridSquare}</a>
                <br>
                Other &nbsp; <a href="https://sdr-list.xyz/" target="new" style="color:rgba(0, 225, 255, 0.993)">Servers</a>
              </div>
        </div>                
      <!--End of Titel Box -->
          ...
          >
            ...
            >
              ...
              <div class="control-group" id="volume-slider">
                ...
                <span class="value-display text-sm text-gray-300 ml-4">{volume}%</span>
              </div>
              ...
              <div class="control-group mt-4" id="squelch-slider">
                ...
                <span class="value-display text-sm text-gray-300 ml-4">{squelch}db</span
                >
              </div>
...
-->
                  ...
                  <div class="flex flex-col items-center"> 
                    <div class="flex space-x-1 mb-1 text-sm">
                      <span
                        class="date-time"
                        style="color:rgba(0, 225, 255, 0.993)"
                        >Time: {time}
                    ...
                    <div class="flex space-x-1 mb-1 text-xs">
                      <div
                        class="px-1 py-0.5 flex items-center justify-center w-10 h-5 relative overflow-hidden"
                      >
                        <span
              ...
              <div id="frequencyContainer" class="w-full mt-4">
                <div class="space-y-5">
                  ...
                  <div class="w-full mb-6"> 
                    ...
                    {#each [{ option: "Auto", AGCbutton : 0 }, { option: "Fast", AGCbutton : 1 }, { option: "Mid", AGCbutton : 2 }, { option: "Slow", AGCbutton : 3 }] 
                    as { option, AGCbutton }} 
                    
                    <button
                    class="retro-button h-8 text-white font-bold h-8 text-sm rounded-md flex items-center justify-center border border-gray-600 shadow-inner transition-all duration-200 ease-in-out {AGCbutton == currentAGC
                    ? 'bg-blue-600 pressed scale-95'
                    : 'bg-gray-700 hover:bg-gray-600'}"
                    on:click={() => {
                    if (option === "Auto") handleAGCChange(0);
                ...
                <div class="w-full mb-6">
                <h3 class="text-white text-base font-semibold mb-2">Filters</h3> 
                 <div id="moreoptions" class="grid grid-cols-4 gap-2 text-sm h-8">
                    {#each [{ option: "NR", icon: "wave-square", enabled: NREnabled }, { option: "NB", icon: "zap", enabled: NBEnabled }, { option: "AN", icon: "shield", enabled: ANEnabled }, { option: "CTCSS", icon: "filter", enabled: CTCSSSupressEnabled }] as { option, icon, enabled }}
                      <button
              ...
              <div class="control-group mt-4" id="audio-buffer-slider">
                <button
                  ...
                  on:click={() =>
                    handleAudioBufferDelayMove((audioBufferDelay += 1))}
                >
                  <span class="text-white text-xs font-normal">Buffer</span>
                </button>
                <div class="slider-container w-10 mx-2">
                  <input
                    type="range"
              ...
              {#if showBookmarkPopup}
                ...
                >
                  ...
                  <div class="mb-6">
                    <label
                      class="block text-sm font-medium text-gray-300 mb-2"
                      >Up-Down/load Bookmarks. REFRESH the webpage (F5) to see them! </label
                    >
                    <div class="flex items-center gap-2">
...
/* --- Floating button (top-right, opaque) --- */
  .shortcuts-side-btn {
  position: fixed;
  top: 5%;
  right: 4.7%;
...
.shortcuts-table td {
  padding: 0.18rem 0.22rem; /* still compact */
  border-bottom: 1px solid #1f2a38;
  font-size: 0.86rem;   /* <-- slightly bigger */
  line-height: 1.22;    /* still tight */
  text-align: left;     /* right column aligned left */
}
...
.shortcuts-table td:first-child {
  width: 65%;   /* <-- updated from 50% to 70% */
