...
export default class SpectrumWaterfall {
  ...
  setCanvasWidth() {
    ...
    this.clientsCanvasElem.height = (canvasWidth / 1372) * 20;
  ...
  adjustWaterfallLimits(bufferedData) {
    if (!this.autoAdjuster) {
      // FIX: Math.min(...bufferedData) and Math.max(...bufferedData) throw
      // RangeError: Maximum call stack size exceeded when bufferedData has
      // more than ~65k elements (which it regularly does after a few frames).
      // That RangeError propagates out of the rAF loop and silently freezes
      // the entire waterfall.  Use a simple loop instead — O(n) and safe.
      let minValue = Infinity;
      let maxValue = -Infinity;
      for (let i = 0; i < bufferedData.length; i++) {
        if (bufferedData[i] < minValue) minValue = bufferedData[i];
        if (bufferedData[i] > maxValue) maxValue = bufferedData[i];
      }
      minValue -= 20;
      maxValue -= 20;
...
//This copied "as is" by sv2amk from below to fix the same issue but for the modes.//
  ...
  drawClients() {
    ...
    const tickLen  = 15    // ← change this number (pixels) to adjust tick length
    const rowStep  = pillH + 3 * scale
    const padTop   = tickLen + 2 * scale  // room for tick above first row
    ...
    const entries = Object.entries(this.clients)
      ...
      .map(([id, range]) => {
        const isMe  = this.myId ? id === this.myId : false
        // If id looks like a real callsign/username (short, ≤12 chars), show it directly.
        // Fall back to 6-digit hash for long random server-assigned IDs.
        const displayId = id.length <= 12 ? id : this._idToSixDigits(id)
        const label = isMe
          ? '★ ' + (this.myUsername || displayId)
          : displayId
        const textW = this.clientsCtx.measureText(label).width
        const pillW = textW + 6 * scale
        const midX  = this.idxToCanvasX(range[1])
        const pillX = midX - pillW / 2
        return { fftOffset: range[1], midX, label, pillW, pillX, isMe }
      })

    // Greedy left-to-right row assignment to avoid overlaps
    ...
    entries.forEach(e => {
      ...
      const tickTop = pillY - tickLen

      this._clientPills.push({ x: e.pillX, y: pillY, w: e.pillW, h: pillH, fftOffset: e.fftOffset })

      // Own pill: white bg + black text; others: black bg + green text
      ...
      this.clientsCtx.strokeStyle = e.isMe ? 'rgba(255,220,0,0.9)' : 'rgba(0,200,0,0.7)'
      this.clientsCtx.lineWidth = 1 * scale
      this.clientsCtx.beginPath()
      this.clientsCtx.roundRect(e.pillX, pillY, e.pillW, pillH, 3 * scale)
      this.clientsCtx.stroke()

      this.clientsCtx.fillStyle = e.isMe ? 'rgba(255,220,0,0.95)' : 'rgba(0,220,0,0.95)'
      this.clientsCtx.textAlign = 'center'
      this.clientsCtx.textBaseline = 'middle'
      this.clientsCtx.fillText(e.label, e.midX, pillY + pillH / 2)

      this.clientsCtx.strokeStyle = 'rgba(255,255,255,0.5)'
      this.clientsCtx.lineWidth = 1 * scale
      this.clientsCtx.beginPath()
      this.clientsCtx.moveTo(e.midX, pillY)
      this.clientsCtx.lineTo(e.midX, tickTop)
      this.clientsCtx.stroke()
    })
  }
