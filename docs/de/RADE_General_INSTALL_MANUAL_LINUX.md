...
**Normalerweise brauchen Sie kein eigenes Steuerskript für RADE.** Die Startskripte (`start-rx888mk2.sh`, `start-airspyhf.sh`, `start-rtl.sh`, `start-rsp1a.sh`) starten das Sidecar selbst, sobald der Server läuft, starten es neu, wenn es sich beendet, und melden seinen Zustand; `stop-websdr.sh` stoppt es zusammen mit dem Server. Gehen Sie direkt zu Schritt 7.
