...
L'essentiel de ce manuel décrit Desktop PhantomSDR+. Le bouton QRG Sync a sa propre section, [QRG Sync sur la page du récepteur](#qrg-sync-sur-la-page-du-récepteur). Le CATsync Tool a sa propre documentation sur son site.
...
## QRG Sync sur la page du récepteur
...
Le bouton se trouve dans la rangée **VFO**, **Modes**, **Bands** et **IF Filters** — **QRG Sync** dans la mise en page large, **CAT** dans la compacte. Son point est **vert** tant qu'un serveur TCI est connecté, **gris** sinon. Il ouvre une fenêtre avec :
...
Ouvrez ensuite la page du récepteur dans un navigateur sur le même ordinateur, ouvrez **QRG Sync** et activez **CAT Sync**. Le point passe au vert et le pont affiche `page connected`. Tournez le VFO et le récepteur suit ; cliquez dans la cascade et le poste suit (`page -> rig ... Hz`) ; passez en émission et la page se tait. **Ctrl+C** arrête le pont, et `rigctl` avec lui.
...
|---|---|
| **SunSDR** avec ExpertSDR2/3, **FlexRadio** avec AetherSDR, **Apache Labs ANAN / Hermes** avec Thetis | Pas de pont. Activez le **serveur TCI** du logiciel et utilisez **QRG Sync** directement — les ports 50001 et 40001 sont trouvés automatiquement |
...
- QRG Sync sur la page du récepteur ne synchronise ni le split, ni le VFO B, ni le RIT/XIT, ni les décalages de transverter, et n'a été essayé qu'avec un IC-7300 via le pont Hamlib.
