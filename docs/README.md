...
### Signal Processing
- **Multiple demodulation modes**: AM, FM, USB, LSB, CW, and more
...
4. **`frontend/src/bands-config.js`** - Band plan configuration
...
### S-meter calibration
The S-meter is already calibrated, but if you wish to re-adjust it, do the following:
- For the **digital S-meter**, adjust it in the .toml file you use. Find 'smeter_offset=' in the [input] section, change it, and restart the server.
- For the **analog S-meter**, edit 'App__analog_smeter_.svelte' and 'App__v2_analog_smeter_.svelte', both located in frontend/src. Search for 'const visualGain = ' and change the value so that the digital and analog S-meters read the same. Then recompile. There is no need to restart the server.


### GUI Variants

Multiple GUI variants are available in the frontend directory. To switch:
- Run recompile.sh in a terminal.
