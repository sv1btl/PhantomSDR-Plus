#!/bin/bash
# ─────────────────────────────────────────────────────────────────────────────
#  PhantomSDR-Plus  –  start-rtl.sh
#  Universal launcher + watchdog for an RTL-SDR front end (rtl_sdr).
#
...
#
#  Usage:
#    ./start-rtl.sh                start (or restart) the server in the background
...
CONFIG="$PHANTOMDIR/config-rtl.toml"
FIFO="$PHANTOMDIR/rtl.fifo"
RX_ARGS="${RX_ARGS:--f 1242000 -s 2048000 -}"
RX_CMD=(rtl_sdr)                                   # binary; args come from RX_ARGS
