...
async init() {
  ...
  _configureAudioLevelMode(mode) {
    ...
    switch (mode) {
      case 1: // Fast — quick attack, moderate hold, pumping-free release
          this.audioLevelEnabled    = true;
          this.audioLevelTarget     = 0.40;
          this.audioLevelMinGain    = 0.68;
          this.audioLevelMaxBoost   = 1.55;
          this.audioLevelAttack     = 0.55;   // was 0.65 — slightly softer; avoids gain-step click
          this.audioLevelRelease    = 0.06;   // was 0.20 — 3× slower; stops inter-word pumping
          this.audioLevelHoldFrames = 4;      // was 1 — ~40 ms hold suppresses gain jump during brief pauses
          this.audioLevelPeakWeight = 0.78;
          this.audioLevelFastCoeff  = 0.28;   // was 0.32 — slightly gentler
          this.audioLevelSlowCoeff  = 0.055;  // was 0.065
          this.audioLevelPeakCoeff  = 0.50;   // fast-tracking smoothed peak (replaces raw per-frame peak)
          break;

      case 2: // Smooth — slow attack/release, no jarring gain steps
          this.audioLevelEnabled    = true;
          this.audioLevelTarget     = 0.40;
          this.audioLevelMinGain    = 0.68;
          this.audioLevelMaxBoost   = 1.55;
          this.audioLevelAttack     = 0.022;  // was 0.018 — marginally more responsive
          this.audioLevelRelease    = 0.010;  // was 0.005 — 2× faster; total recovery ~300 ms instead of ~1 s
          this.audioLevelHoldFrames = 18;     // was 55 (~550 ms freeze); now ~180 ms — smooth but not stuck
          this.audioLevelPeakWeight = 0.78;
          this.audioLevelFastCoeff  = 0.08;   // was 0.07
          this.audioLevelSlowCoeff  = 0.016;  // was 0.014
          this.audioLevelPeakCoeff  = 0.12;   // slow-tracking smoothed peak follower
          break;

      case 3: // Adaptive — timing adjusts to signal character (SSB/CW/AM)
          this.audioLevelEnabled    = true;
          this.audioLevelTarget     = 0.40;
          this.audioLevelMinGain    = 0.68;
          this.audioLevelMaxBoost   = 1.55;
          this.audioLevelAttack     = 0.13;
          this.audioLevelRelease    = 0.028;
          this.audioLevelHoldFrames = 10;
          this.audioLevelPeakWeight = 0.78;
          this.audioLevelFastCoeff  = 0.18;
          this.audioLevelSlowCoeff  = 0.040;
          this.audioLevelPeakCoeff  = 0.22;   // medium-speed smoothed peak follower
          break;
...
_updateCTCSSGate(rawPcm) {
  ...
  initTimer() {
    ...
    this.ft4CollectionTimer = setInterval(() => {
        ...
        this.updateFT2CollectionStatus();
    }, 250);
...
// ── FT2 collection ──────────────────────────────────────────────────────
updateFT2CollectionStatus() {
    if (!this.decodeFT2) return;
    const now       = new Date();
    const msInMin   = (now.getSeconds() * 1000) + now.getMilliseconds();
    const slotMs    = 3750;
    const posInSlot = msInMin % slotMs;
    if (posInSlot < 400 && !this.isFT2Collecting) {
        this.startFT2Collection();
    } else if (posInSlot > 3500 && this.isFT2Collecting) {
...
async stopFT2Collection() {
    ...
    let decodedMessages;
    try {
        decodedMessages = await _workerDecode('ft2', pcm);
    } catch (e) {
