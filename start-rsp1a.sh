...
#    RX_DRIVER=miri|sdrplay       force a driver instead of detecting it
#
#  Drivers: ./setup-rsp1a.sh (or install.sh option 3) installs the open one,
#  libmirisdr-5 + SoapyMiri. SDRplay's own API (instructions-for-rsp1a) works
#  too — see RX_DRIVER in the configuration block.
#
#  NOTE: not tested on RSP1A hardware — it reuses the exact control/watchdog
#  logic validated on RX-888; only the receiver command/config differ. On the
#  SDRplay API, the sdrplay service restart in prestart() needs root /
#  passwordless sudo.
# ─────────────────────────────────────────────────────────────────────────────
...
# Two drivers can run the RSP1A:
#   miri     libmirisdr-5 + SoapyMiri — open source, no service, what
#            ./setup-rsp1a.sh installs
#   sdrplay  SDRplay's closed API + SoapySDRPlay (instructions-for-rsp1a),
#            which needs its sdrplay service running
...
prestart() {
    local drv="$RX_DRIVER"
    if [ "$drv" = "auto" ]; then
        if SoapySDRUtil --info 2>/dev/null | grep -i 'factories' | grep -qw sdrplay; then
            drv="sdrplay"
        else
            drv="miri"
        fi
    ...
    if [ -z "$RX_ARGS" ]; then
        case "$drv" in
            sdrplay) RX_ARGS="$RX_ARGS_SDRPLAY" ;;
            *)       RX_ARGS="$RX_ARGS_MIRI" ;;
        esac
    fi
    log "RSP1A driver: $drv"
    [ "$drv" = "sdrplay" ] || return 0
    # The SDRplay API needs its service running. Best-effort; needs root /
    # passwordless sudo (a password prompt here would hang the watchdog, so
    # it's suppressed).
