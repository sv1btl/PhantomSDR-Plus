...
export class KiwiSSTVDecoder {
  ...
  reset(opts = {}) {
    ...
    // Transposed direct-form II; coefficients for Fs = 12000 Hz.
    // Passband ~1000–2500 Hz covers all SSTV and Robot 36 signal content.
    //
    // Stage 1: Highpass Butterworth  f0 = 1000 Hz,  Q = 1/√2
    //   b = [ 0.68932, -1.37864,  0.68932 ]
    //   a = [ 1.00000, -1.27955,  0.47759 ]
    this._hp_b = [ 0.68932, -1.37864,  0.68932];
    this._hp_a = [-1.27955,  0.47759];
    this._hp_z = [0.0, 0.0];

    // Stage 2: Lowpass Butterworth   f0 = 2500 Hz,  Q = 1/√2
    //   b = [ 0.22020,  0.44041,  0.22020 ]
    //   a = [ 1.00000, -0.30759,  0.18836 ]
    this._lp_b = [ 0.22020,  0.44041,  0.22020];
    this._lp_a = [-0.30759,  0.18836];
    this._lp_z = [0.0, 0.0];
    ...
    this._r36_pendingCb   = null;   // Uint8Array(320) or null
    ...
    this._modes = {
      ...
      scottieDX: {
        ...
        family: 'scottie',
        syncMs: 9.0, porchMs: 1.5, sepMs: 1.5,
        chanMs: 345.6, lineMs: 1051.8, order: ['g', 'b', 'r']
      },

      // ── YCbCr modes (Robot family) ───────────────────────────────────────
      // Line structure: sync(9ms@1200Hz) + porch(3ms@1500Hz) + Y(yMs) +
      //                 sep(6ms@1500Hz) + C(cMs)
      // Cb (B−Y) on even lines, Cr (R−Y) on odd lines.
      // Robot 36 and Robot 72 share the same decoder (_decodeLineRobot36).
      robot36: {
      ...
      robot72: {
        // Robot 72 — doubled scan times vs Robot 36 (same decoder family).
        // yMs = 2 × 88 = 176 ms; cMs = 2 × 44 = 88 ms; line = 282 ms.
        // 240 lines ≈ 67.7 s total.  Some references define 300 ms/line
        // (yMs=188/cMs=94) for exactly 72 s; 282 ms is the canonical 2× value.
        name: 'Robot 72', vis: 12, width: 320, height: 240,
        family: 'robot36',    // reuse Robot 36 YCbCr decoder unchanged
        syncMs: 9.0, porchMs: 3.0, sepMs: 6.0,
        yMs: 176.0, cMs: 88.0, lineMs: 282.0
      }
    };
  ...
  feedPCM(pcm) {
    ...
    for (let i = 0; i < pcm.length; i++) {
      const cur = pcm[i];
      // Resample to 12 kHz first (linear interpolation), then apply the
      // bandpass filter at the decode rate so the IIR coefficients are correct.
      while (phase <= 1.0) {
    ...
    for (const key of candidateKeys) {
      ...
      for (let a = anchorMin; a <= anchorMax; a += step) {
        ...
        for (let n = 0; n < 4; n++) {
          ...
          const p1500 = this._goertzel(syncStart, syncS, 1500);
          const p1900 = this._goertzel(syncStart, syncS, 1900);
          score += (p1200 - 0.28 * p1500 - 0.42 * p1900) /
  ...
  _findSyncNear(expectedSyncStart) {
    ...
    // Measure it with the raw arccos estimator and accumulate the error via
    // a slow EMA (α = 0.20).  Applied to all subsequent pixel estimates.
    if (bestQ > 0 && bestPos >= 0 && bestPos + syncS <= this._bufLen) {
      const measured = this._estimateFreqRaw(bestPos, syncS);
  ...
  // Normalised lag-1 autocorrelation estimator.
  //
  // For x[n] = A·cos(2πf·n/sr):
  //   Σ x[n]·x[n-1] / Σ x[n]²  ≈  cos(2πf/sr)
  //   ∴  f = arccos(r1/r0) · sr / 2π
  //
  // Amplitude-independent, continuous, stable for windows ≥ 4 samples.
  _estimateFreqRaw(start, len) {
    const begin = Math.max(1, Math.floor(start));
    const end   = Math.min(this._bufLen, Math.floor(start + len));
    if (end - begin < 4) return 1500;
    let r0 = 0, r1 = 0;
    for (let i = begin; i < end; i++) {
      r0 += this._buf[i]     * this._buf[i];
      r1 += this._buf[i] * this._buf[i - 1];
    }
    if (r0 < 1e-12) return 1500;
    const cosW = Math.max(-1.0, Math.min(1.0, r1 / r0));
    return Math.acos(cosW) * this._decodeSps / (2.0 * Math.PI);
  }

  // Calibrated estimate: raw frequency + accumulated sync offset.
  _estimateFreq(start, len) {
    return this._estimateFreqRaw(start, len) + this._freqOffset;
  }
  ...
  _decodeLine(lineStart, syncStart) {
    ...
    // Estimation window: fixed at N=8 samples for all RGB modes.
    // At 12 kHz, 8 samples = exactly one period of 1500 Hz (black), so the
    // double-frequency cross-term in the arccos autocorrelation estimator
    // cancels perfectly.  This eliminates the large variance (σ≈52 px at
    // 1500 Hz with N=6) that was making dark areas look grey and whites dark.
    // RMS pixel error across 1500–2300 Hz: 1.9 px (vs 17.3 px for N=6).
    const estLen    = 8;
    const halfExtra = Math.max(0, (estLen - pixSpan) * 0.5);

    // FIX [16]: clamp estimation window to channel bounds.
    // Without the clamp, at x=0 the window starts halfExtra samples *before*
    // the channel (in the separator or porch), systematically darkening the
    // leftmost pixels in modes where estLen > pixSpan (M2, S2, SDX chroma).
    const rEnd = segments.r + chS - estLen;
    const gEnd = segments.g + chS - estLen;
    const bEnd = segments.b + chS - estLen;

    for (let x = 0; x < W; x++) {
      const rS = Math.max(segments.r, Math.min(rEnd, segments.r + x * pixSpan - halfExtra));
      const gS = Math.max(segments.g, Math.min(gEnd, segments.g + x * pixSpan - halfExtra));
      const bS = Math.max(segments.b, Math.min(bEnd, segments.b + x * pixSpan - halfExtra));

      // 1500 Hz = black (0),  2300 Hz = white (255),  range = 800 Hz.
      // Gamma LUT applied — see _buildGammaLUT() / setGamma().
      const rv = this._gammaLUT[Math.max(0, Math.min(255, Math.round((this._estimateFreq(rS, estLen) - 1500) * 255 / 800)))];
      const gv = this._gammaLUT[Math.max(0, Math.min(255, Math.round((this._estimateFreq(gS, estLen) - 1500) * 255 / 800)))];
      const bv = this._gammaLUT[Math.max(0, Math.min(255, Math.round((this._estimateFreq(bS, estLen) - 1500) * 255 / 800)))];
  ...
  // ── Robot 36 / Robot 72 YCbCr line decoder ───────────────────────────────
  //
  // Handles both Robot 36 (VIS=8) and Robot 72 (VIS=12).  The two modes share
  // the same line structure — Robot 72 simply doubles the Y and C scan times.
  // The mode object (this._mode) carries the correct yMs/cMs/syncMs values so
  // the decoder is fully generic.
  //
  // Line structure (offsets from syncStart):
  //   sync  : syncMs  @ 1200 Hz   (Robot 36: 108 s; Robot 72: 108 s — same)
  //   porch : porchMs @ 1500 Hz   (Robot 36:  36 s; Robot 72:  36 s — same)
  //   Y scan: yMs                 (Robot 36: 1056 s; Robot 72: 2112 s)
  //   sep   : sepMs   @ 1500 Hz   (Robot 36:  72 s; Robot 72:  72 s — same)
  //   C scan: cMs                 (Robot 36:  528 s; Robot 72: 1056 s)
  //
  // Returns an array of {pixels: Uint8ClampedArray, lineNum: number} objects:
  //   • Even line (Cb): buffers Y+Cb, returns []   (waits for Cr from next line)
  //   • Odd  line (Cr): emits [{lineNum:n-1}, {lineNum:n}]  (pair complete)
  //
  // This._line is the current line number in the frame (0-indexed).
  _decodeLineRobot36(lineStart, syncStart) {
    const m   = this._mode;
    const W   = m.width;                                  // 320
    const sr  = this._decodeSps;

    const syncS  = Math.round(m.syncMs  * sr / 1000);    // 108
    const porchS = Math.round(m.porchMs * sr / 1000);    //  36
    const yS     = Math.round(m.yMs     * sr / 1000);    // 1056
    const sepS   = Math.round(m.sepMs   * sr / 1000);    //  72
    const cS     = Math.round(m.cMs     * sr / 1000);    // 528

    const yStart = syncStart + syncS + porchS;           // start of Y scan
    const cStart = yStart + yS + sepS;                   // start of C scan

    const pixSpanY  = yS / W;                            // 3.3 samples/pixel (R36); 6.6 (R72)
    const pixSpanC  = cS / W;                            // 1.65 samples/pixel (R36); 3.3 (R72)
    // N=8 for both Y and C: same resonance benefit as the RGB decoder.
    // Chroma blurs slightly (4.8 C-pixels per estimate) but chroma noise is
    // far more objectionable than chroma softness for natural images.
    const estLenY   = 8;
    const estLenC   = 8;
    const halfY     = Math.max(0, (estLenY - pixSpanY) * 0.5);
    const halfC     = Math.max(0, (estLenC - pixSpanC) * 0.5);

    // FIX [16]: clamp estimation windows to their channel bounds.
    const yEndBound = yStart + yS - estLenY;
    const cEndBound = cStart + cS - estLenC;

    // Decode Y channel (same mapping as RGB luma: 1500 Hz=0, 2300 Hz=255)
    const Y = new Uint8Array(W);
    for (let x = 0; x < W; x++) {
      const s = Math.max(yStart, Math.min(yEndBound, yStart + x * pixSpanY - halfY));
      Y[x] = Math.max(0, Math.min(255,
        Math.round((this._estimateFreq(s, estLenY) - 1500) * 255 / 800)
      ));
    }

    // Decode chroma channel (same Hz→level mapping; 128 = neutral)
    const C = new Uint8Array(W);
    for (let x = 0; x < W; x++) {
      const s = Math.max(cStart, Math.min(cEndBound, cStart + x * pixSpanC - halfC));
      C[x] = Math.max(0, Math.min(255,
        Math.round((this._estimateFreq(s, estLenC) - 1500) * 255 / 800)
      ));
    }
    ...
    if (isEven) {
      // Even line carries Cb (B−Y).  Buffer Y and Cb; wait for Cr.
      this._r36_pendingY    = Y;
      this._r36_pendingCb   = C;   // C is Cb on even lines
      this._r36_pendingLine = lineNum;
      return [];
    }

    // Odd line carries Cr (R−Y).  Pair with buffered even-line Cb.
    const Cr = C;
    const Cb = this._r36_pendingCb || new Uint8Array(W).fill(128);  // 128 = neutral if no prior even line
    const prevY    = this._r36_pendingY;
    const prevLine = this._r36_pendingLine;

    const rows = [];

    // Emit the buffered even row (now that we have its Cr from this odd line)
    if (prevY) {
      rows.push({ pixels: this._yuv2rgb(prevY, Cb, Cr), lineNum: prevLine });
    }

    // Emit the current odd row (shares Cb from the preceding even line)
    rows.push({ pixels: this._yuv2rgb(Y, Cb, Cr), lineNum });
  ...
  _process() {
    ...
    while (
      ...
      // Decode line and emit.  Robot 36 returns 0 rows (even line buffered)
      // or 2 rows (odd line: the buffered even + current odd).  All other
      // families return exactly 1 row wrapped in an array for uniformity.
      let rows;
      if (m.family === 'robot36') {
        rows = this._decodeLineRobot36(lineStart, syncStart);
      } else {
