...
Seul `/audio` est vérifié, car c'est là que vit une session, et la boucle locale ainsi que les chemins Kiwi sont exemptés afin que le tap autorun et les clients Kiwi ne soient pas touchés. `0` le désactive et c'est la valeur par défaut ; remettez-le à `0` une fois les anciennes pages écoulées, car il éconduit aussi les clients de diversité des autres stations, qui n'envoient pas le marqueur.
